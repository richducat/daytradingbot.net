...
export class SmtpLicenseMailer implements LicenseMailer {
  ...
  async sendLicense(input: {
    ...
    await this.transport.sendMail({
      from: this.from,
      to: input.to,
      subject: "Your DayTradingBot access is ready",
      text: [
        "You're in. Your access code unlocks your DayTradingBot account.",
        "",
        `Your private access code: ${input.activationCode}`,
        "",
        ...downloads,
        "",
        "What to do next:",
        "1. Open your DayTradingBot account in the browser.",
        "2. Paste the access code above.",
        "3. Download the Mac app when you're ready to connect Robinhood.",
        "4. Open Bluechip in Practice first, then choose Real in the Mac app only when you're ready.",
        "",
        "Your Robinhood connection and trading stay on your Mac. The website never receives your Robinhood login, account details, or trades.",
        `Need help? Email ${this.supportEmail}.`,
        "",
        "Real trading can lose money. Bluechip does not promise a profit, and you decide when to use it.",
      ].join("\n"),
    });
  }
}
