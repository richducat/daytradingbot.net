...
export class SmtpLicenseMailer implements LicenseMailer {
  ...
  async sendLicense(input: {
    ...
    await this.transport.sendMail({
      from: this.from,
      to: input.to,
      subject: "Your Bluechip dashboard is ready",
      text: [
        "You're in. Bluechip is ready to open in your browser.",
        "",
        ...
        "3. Connect your dedicated Robinhood Agentic account.",
        "4. Review the dollar limits and start Bluechip in Practice.",
        "",
        "Practice shows current decisions without placing an order. You decide if and when to use Real trading.",
        `Need help? Email ${this.supportEmail}.`,
        "",
        "Real trading can lose money. Bluechip does not promise a profit. Your trading money stays in Robinhood.",
      ].join("\n"),
    });
  }
}
